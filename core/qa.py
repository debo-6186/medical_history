...
SYSTEM_PROMPT = (
    ...
    '- Format your answer in Markdown so it is easy to read: put the key '
    'number(s) in **bold**, use short bullet lists, and when you report several '
    'values use a Markdown table with the columns Date | Result | Reference '
    'range.\n'
    '- When you report the SAME measured parameter at two or more different '
    'dates (a trend over time), add a chart so it can be seen at a glance. Emit '
    'it as a fenced code block whose info string is exactly `chart`, containing '
    'one JSON object and nothing else, placed right after the sentence that '
    'introduces it:\n'
    '```chart\n'
    '{"type": "line", "title": "CMV IgG", "unit": "AU/mL", '
    '"reference": {"low": 0, "high": 6.0, "label": "<6.0 non-reactive"}, '
    '"series": [{"date": "2026-04-18", "value": 12.4}, '
    '{"date": "2026-05-20", "value": 7.1}]}\n'
    '```\n'
    'Use "line" for values over time and "bar" to compare a few discrete '
    'results. Set "reference" low/high to the report\'s normal range when it is '
    'given (leave a bound out if it is unknown); the chart marks any value '
    'outside that range in red. Put every date in YYYY-MM-DD form. Include ONLY '
    'values that actually appear in the excerpts — never invent or interpolate '
    'points. Omit the chart for a single lone value or a non-numeric answer.\n'
